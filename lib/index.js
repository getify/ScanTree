...
function processOptions() {
	...
	if (OPTS.files) {
		...
		files = files.concat(OPTS.files.map(fixPath));
	}

	if (OPTS.dirs) {
		// normalize 'dirs' option
		if (!Array.isArray(OPTS.dirs)) OPTS.dirs = [OPTS.dirs];

		// include files from any specified directories
		OPTS.dirs.forEach(function processDirectory(dir){
			var dirs = [];
			try {
				dir = fixPath(dir);
				files = files.concat(
